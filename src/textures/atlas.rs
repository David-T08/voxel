...
impl<I> Default for UnbuiltAtlas<I> where 
    I: RegistryId
{
    fn default() -> Self {
        Self { entries: Vec::new(), ready: false, _marker: PhantomData }
    }
}
    ...
    I: RegistryId + Clone
{
    ...
    pub fn ready_status(&self, assets: &AssetServer, images: &Assets<Image>) -> AtlasReadyStatus {
        if !self.ready {
            return AtlasReadyStatus { ready: false, pending: 0, loaded: u16::MAX, total: 0 }
        }
    ...
    pub fn verify(&self, images: &Assets<Image>) -> Result<u32, Vec<AtlasError<I>>> {
        ...
        for (id, tex) in self.entries.iter().enumerate() {
            ...
            if w > MAXIMUM_TEXTURE_SIZE || h > MAXIMUM_TEXTURE_SIZE || w < MINIMUM_TEXTURE_SIZE || h < MINIMUM_TEXTURE_SIZE {
    ...
    pub fn build(self, assets: &AssetServer, images: &mut Assets<Image>) -> Result<BuiltAtlas<I>, (Vec<AtlasError<I>>, UnbuiltAtlas<I>)> {
...
impl<I> BuiltAtlas<I> where 
    I: Clone + RegistryId
{
    pub fn get(&self, id: I) -> Result<&AtlasRegion, AtlasError<I>> {
        self.entries.get(id.to_index()).ok_or(AtlasError::MissingId(id))
    }
