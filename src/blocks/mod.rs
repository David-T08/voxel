...
fn load_block_definitions(
    ...
    for entry in asset_dir {
        ...
        let value = parse_to_serde_value(&text, &Default::default()).unwrap().unwrap();
        ...
        match block_registry.register_from_asset(name.clone(), data.textures, &tex_registry) {
          Some(_) => println!("Added blocks/{stem} to block registry, registered as: {:?}", block_registry.names.name_to_id(&name)),
